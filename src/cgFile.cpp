...
void check(bool state, string &&errStr, string &&okStr)
{
    if (!state)
    {
        if (!errStr.empty()) { std::cerr << (errStr.c_str()); }
        exit(EXIT_FAILURE);
    }
    else
    {
...
void check(const int runCode, string &&errStr, string &&okStr)
{
    if (runCode != CG_OK)
    {
        if (!errStr.empty()) { std::cerr << "ERROR: " << (errStr.c_str()); }
        exit(EXIT_FAILURE);
    }
    else
    {
...
const unordered_map<ElementType_t, vector<vector<int>>> CGFile::nodeIdToBuildFaceInCell = {{ElementType_t::TETRA_4, {{0, 2, 1}, {0, 1, 3}, {1, 2, 3}, {2, 0, 3}}},
                                                                                           {ElementType_t::HEXA_8, {{0, 3, 2, 1}, {0, 1, 5, 4}, {1, 2, 6, 5}, {2, 3, 7, 6}, {0, 4, 7, 3}, {4, 5, 6, 7}}},
                                                                                           {ElementType_t::PENTA_6, {{0, 2, 1}, {0, 1, 4, 3}, {1, 2, 5, 4}, {2, 0, 3, 5}, {3, 4, 5}}},
                                                                                           {ElementType_t::PYRA_5, {{0, 3, 2, 1}, {0, 1, 4}, {1, 2, 4}, {2, 3, 4}, {3, 0, 4}}}};

CGFile::CellLoader::CellLoader(Section &curS, const int fp, const cgsize_t lowerBd, const cgsize_t upperBd) : curS(curS), fp(fp)
{
...
const vector<cgsize_t> &CGFile::Section::cell(const int id) const { return data[IDMap.at(id)]; }
...
cgsize_t CGFile::Section::flag(const int id) const { return isMixed() ? typeFlag[IDMap.at(id)] : (cgsize_t)cellType; }

bool CGFile::Section::isMixed() const { return (cellType == CGNS_ENUMV(MIXED)); }
...
CGFile::Section CGFile::Section::subSection(const cgsize_t beg, const cgsize_t end)
{
    ...
    for (auto id = beg; id <= end; ++id) { rst.addCell(id, this->cell(id), this->flag(id)); }
...
const vector<int> &CGFile::bodySectionIdList() const { return bodySectionIDList_; }

const vector<int> &CGFile::bdySectionIdList() const { return bdySectionIDList_; }
...
void CGFile::checkFile()
{
    ...
    int filetype;
    ZoneType_t zoneType;
    check(cg_is_cgns(filename_.c_str(), &filetype), format("%s is not a CGNS file\n", filename_.c_str()));

    check((cg_set_file_type(CG_FILE_ADF) == CG_OK && cg_open(filename_.c_str(), CG_MODE_READ, &fp) == CG_OK) || (cg_set_file_type(CG_FILE_HDF5) == CG_OK && cg_open(filename_.c_str(), CG_MODE_READ, &fp) == CG_OK), format("can not open file: %s \n", filename_.c_str()));
    isOpen_ = true;

    check((cg_nbases(fp, &nbases), nbases == 1), format("now only support one Base, you have %d\n", nbases));

    check((cg_base_read(fp, ibase, basename, &cell_dim_, &phys_dim), phys_dim == 3), format("now only support 3-Dim mesh, you have %d\n", phys_dim));

    check((cg_nzones(fp, ibase, &nzones), nzones == 1), format("now only support one Zone, you have %d\n", nzones));

    check((cg_zone_type(fp, ibase, izone, &zoneType), zoneType == ZoneType_t::Unstructured), format("now only support UnStructured Mesh\n"));

    check(cg_zone_read(fp, ibase, izone, zonename, zoneInfo));

    check((cg_ngrids(fp, ibase, izone, &ngrids), ngrids == 1), format("now only support single layer Mesh, you have %d\n", ngrids));

    int nSection;
    Section section;
    check((cg_nsections(fp, ibase, izone, &nSection), nSection > 0), format("no section found in file %s", filename_.c_str()));
    for (int iSection = 1; iSection <= nSection; ++iSection)
    {
        ...
        if (this->isBodySection(section))
            bodySectionIDList_.push_back(iSection);
        else
            bdySectionIDList_.push_back(iSection);
...
ElementType_t CGFile::CellType(const int nodeCnt, const int dim)
{
    ...
    case 2:
        switch (nodeCnt)
        {
        case 3: return ElementType_t::TRI_3; break;
        case 4: return ElementType_t::QUAD_4; break;
        default: std::cout << format("not supported element type with[dim, nodes] [ %d, %d]\n", dim, nodeCnt); break;
        }
        break;
    case 3:
        switch (nodeCnt)
        {
        case 4: return ElementType_t::TETRA_4; break;
        case 5: return ElementType_t::PYRA_5; break;
        case 6: return ElementType_t::PENTA_6; break;
        case 8: return ElementType_t::HEXA_8; break;
        default: std::cout << format("not supported element type with[dim, nodes] [ %d, %d]\n", dim, nodeCnt); break;
        }
        break;
    default: break;
...
ElementType_t CGFile::CellType(const int CGNSCellTypeFalg)
{
    ...
    {
    case 5: rst = ElementType_t::TRI_3; break;
    case 7: rst = ElementType_t::QUAD_4; break;
    case 10: rst = ElementType_t::TETRA_4; break;
    case 12: rst = ElementType_t::PYRA_5; break;
    case 14: rst = ElementType_t::PENTA_6; break;
    case 17: rst = ElementType_t::HEXA_8; break;
    default: std::cout << format("unknown element type %d\n", static_cast<ElementType_t>(CGNSCellTypeFalg)); break;
    }
    return rst;
}
...
bool CGFile::isBodySection(const Section &section)
{
    ...
    {
    case NODE: cell_dim = 0; break;
    case BAR_2: cell_dim = 1; break;
    case TRI_3:
    case QUAD_4: cell_dim = 2; break;
    case TETRA_4:
    case PYRA_5:
    case PENTA_6:
    case HEXA_8: cell_dim = 3; break;
    case MIXED:
        cgsize_t flag;
        ...
        case ElementType_t::QUAD_25: cell_dim = 2; break;
        default: cell_dim = 3; break;
        }
        break;
    default: std::cerr << ("Unsupported element type\n"); break;
    }
...
void CGFile::loadCGIOInfo()
{
    ...
    auto lookupNodeIdByLabel = [&](double nodeId, string label)
    {
        ...
        for (auto id : getChildrenIds(nodeId))
        {
            ...
            if (string(tmpLabel) == label) { rst.push_back(id); }
        }
...
CGFile::Section &CGFile::loadSection(const int id)
{
    ...
    while (loader.nextCell(tmp)) { curSection.addCell(ID++, tmp, loader.flag); }
...
CGFile::Section CGFile::loadSection(const vector<int> idList)
{
    ...
    for (auto bodyId : idList)
    {
        ...
        while (loader.nextCell(tmp)) { curS.addCell(ID++, tmp, loader.flag); }
    }
...
cgsize_t CGFile::nCell() const { return zoneInfo[1]; }

void CGFile::nCell(const cgsize_t ncell) { zoneInfo[1] = ncell; }

cgsize_t CGFile::nNode() const { return zoneInfo[0]; }

void CGFile::nNode(const cgsize_t nnode) { zoneInfo[0] = nnode; }
...
void CGFile::writeCoordinate(const vector<vector<double>> &data)
{
    ...
    // base
    check(cg_base_write(fp, "CGNSBASE", 3, 3, &dummy), "write base\n");

    // zone
    check(cg_zone_write(fp, 1, "ZONE", zoneInfo, Unstructured, &dummy), "write zone\n");

    // grid
    check(cg_grid_write(fp, 1, 1, "GridCoordinates", &igrid), "write grid\n");
...
{
    // update id
    curS.dataSize = std::accumulate(curS.data.begin(), curS.data.end(), 0, [](cgsize_t sum, vector<cgsize_t> vec) { return sum + vec.size(); });
    ...
    if (curS.isMixed())
    {
        ...
        for (auto it : curS.data)
        {
            ...
            for (auto j : it) { data[i++] = nodeIdG2L.at(j); }
        }
        check(cg_poly_section_write(fp, 1, 1, curS.name, curS.cellType, curS.start, curS.end, 0, data.data(), curS.offset.data(), &dummy), format("file %s write section %s\n", filename_.c_str(), curS.name));
    }
    else
    {
        ...
        check(cg_section_write(fp, 1, 1, curS.name, curS.cellType, curS.start, curS.end, curS.nBdy, data.data(), &dummy), format("file %s write section %s\n", filename_.c_str(), curS.name));
    }
