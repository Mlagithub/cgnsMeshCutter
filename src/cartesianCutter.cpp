...
{   
    ...
    for(int iSection = 1; iSection<=nSection; ++iSection)
    {
        ...
        if(!interiorFacePrefix_.empty() && stringRegexFind(interiorFacePrefix_, section.name)) interiorSection_.push_back(iSection);
...
void CartesianCutter::cut(int argc, char** argv)
{
    CmdLine cl{};
...
{
    // check parameter
    if(multiZone) check(!fluidDomainNameRule.empty(), format("multiZone mode should offer fluid domain names\n"));
    interiorFacePrefix_ = interiorSection;
    ...
    {
        set<int> sectionInBox, solidInBox, fluidInBox;
        ...
        for (auto iBody : bodySection_)
        {
            ...
            if(fluidDomainNameRule.empty())
            {
                fluidInBox.insert(iBody);
            }
            else
            {
                if(this->isFluidDomain(curS.name, fluidDomainNameRule)) fluidInBox.insert(iBody);
                else solidInBox.insert(iBody);
            }
            sectionInBox.insert(iBody);

            updateNodeId(curS);
        }
        if(fluidInBox.empty()) check(false, format("can not find fluid %s, check your input args\n", (std::accumulate(std::next(fluidDomainNameRule.begin()), fluidDomainNameRule.end(), *fluidDomainNameRule.begin(), [](string sum, const string& val){ return sum+","+val;})).c_str()));
        ...
        // clear
        for(auto i = 1; i<=iFile; ++i)
        {
            if(outerFace_[i].empty())
            {
                nodeIdMap_[i].clear();
                if(smallFiles_.find(i)!=smallFiles_.end())
                {
                    check(cg_close(smallFiles_[i]), "close smallFile\n");
                    smallFiles_.erase(i);
                }
            }
        }
...
bool CartesianCutter::isFluidDomain(const string& name, const vector<string>& fluidDomainNameRule)
{
    for(auto it : fluidDomainNameRule)
    {
        if(stringRegexFind(it, name)) return true;
    }
    return false;
}
